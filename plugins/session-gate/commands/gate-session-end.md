---
description: Thin closeout wrapper for KB/Wiki vNext, KB-lifecycle, and a companion workflow plugin
...
not fabricate missing handoffs, KB records, or companion workflow state.
...
- If vNext, KB, and the companion workflow are all absent, stop after a graceful closeout message.
...
2. Companion workflow audit: list only unresolved session, review, or handoff
   items that actually exist in the workspace.
...
## Step 5: Companion workflow closeout (only if `case.found == true`)

If `case.found` is `false`, skip this entire section. Do not mention the
companion workflow, handoffs, role boundaries, ALLOW/BLOCK, or companion
workflow cleanup in the final closeout.

Do not update `companion_state.json`. That file is owned by the companion
workflow plugin. Session-gate only reads it for detection purposes.

When the companion workflow is present:

1. Use the canonical companion workflow `/handoff` contract resolved by the detector.
...
- `Companion workflow closeout` only if the companion workflow was detected
- `Consistency` only if both were detected
- `Safe to close` conclusion

For KB-only and empty workspaces, a mention of the companion workflow is a regression.
